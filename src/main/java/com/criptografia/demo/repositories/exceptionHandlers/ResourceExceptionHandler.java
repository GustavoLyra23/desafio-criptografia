package com.criptografia.demo.repositories.exceptionHandlers;

import com.criptografia.demo.dto.CustomError;
import com.criptografia.demo.dto.ValidationError;
...
public class ResourceExceptionHandler {
    ...
    public ResponseEntity<CustomError> resourceNotFound(MethodArgumentNotValidException e, HttpServletRequest request) {
        ...
        e.getBindingResult().getFieldErrors()
                .forEach(x -> err.addFieldError(x.getField(), x.getDefaultMessage()));
        return ResponseEntity.status(status).body(err);
    }
}
